<template>
  <div>
    ...
    <div>
      ...
      <!-- 기간 수동 입력 -> 굳이..? -->
      <!-- <label>
        <input type="number" v-model.number="minRate" placeholder="최소" style="width: 60px" />
        ~
        <input type="number" v-model.number="maxRate" placeholder="최대" style="width: 60px" />
      </label> -->
    </div>
    ...
    <table v-if="products.length" border="1" style="width: 100%; border-collapse: collapse">
      ...
      <tbody>
        ...
        :key="product.fin_prdt_cd + '-' + product.fin_co_no"
    ...
    <!-- 모달 블록 -->
    <div v-if="showModal" style="position: fixed; top: 20%; left: 50%; transform: translateX(-50%); background: white; border: 1px solid #ccc; padding: 1rem; z-index: 1000; width: 600px">
      <h3>상품 세부 정보</h3>
      <!-- 이자 지급 방식 랜더링 수정해야 함 -->
      ...
      <button @click="closeModal" style="margin-top: 1rem">닫기</button>
    </div>
